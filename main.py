...
def get_user_info(website_url, user_id):
    response = requests.get(f"{website_url}/wp-json/wp/v2/users/{user_id}")
    if response.status_code == 200:
        return response.json()
    else:
        return None

def get_authors(website_url):
    response = requests.get(f"{website_url}/wp-json/wp/v2/users")
    if response.status_code == 200:
        return response.json()
    else:
        return None
...
def main():
    ...
    for user_id in range(1, num_users + 1):
        ...
        user_info = get_user_info(website_url, user_id)
    ...
    authors = get_authors(website_url)
